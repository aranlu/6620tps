...
int ejecutar_desde_archivo(char* nombre,tda_stack* pila){
	...
	archivo=(char*)malloc(size*sizeof(char));

	fread(archivo,1,size,a);
	archivo[size-1]='\0';

	fclose(a);

	ejecutar(archivo,pila);
...
int leerEntradaStd(tda_stack* pila){
	...
	if ( datosEntrada.data == NULL ){
		fprintf(stderr, "ERROR: No hay recursos para comenzar el procesamiento...\n");
		return (EXIT_FAILURE);
	}
	...
	do{
		...
		if ( datosEntrada.length == datosEntrada.allocated ){
			aux=datosEntrada.data; /*1: agregue esto*/
			datosEntrada.data = (char *) realloc(datosEntrada.data, (BUFFER_SIZE * ((datosEntrada.length / BUFFER_SIZE) + 1)) * sizeof(char) );
			if ( datosEntrada.data == NULL ){
				fprintf(stderr, "ERROR: No hay recursos para continuar el procesamiento...\n");
				free(aux); /*2: cambie esto*/
				aux=NULL;
				return(EXIT_FAILURE);
			}
			datosEntrada.allocated += BUFFER_SIZE;
